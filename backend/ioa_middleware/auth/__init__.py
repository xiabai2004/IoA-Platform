...
_OPEN_PREFIXES = (
    ...
    "/health",
    "/gui",
    "/a2a",  # A2A 协议端点（Agent Card 发现需要开放）
)
...
class TokenAuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if scope["type"] == "websocket":
            ...
            # 开发环境允许无 token 连接（生产环境应移除此逻辑）
            if token and token != _PRE_SHARED_KEY:
                await self._unauthorized_ws(send, "Invalid token")
                return
