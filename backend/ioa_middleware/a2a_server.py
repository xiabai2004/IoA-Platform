...
class A2AToIoAPBridge:
    ...
    实现 A2A 和 IoAP 协议之间的双向转换：
    - A2A Task → IoAP 消息 → Agent 执行
    - Agent 结果 → IoAP 消息 → A2A Task 状态更新

    支持：
    - 双向消息映射
    - Task 状态同步（A2A TaskState ↔ IoAP DAG Status）
    - 结构化数据传递（不只是文本）
    """

    def __init__(self, ioap_send_func, dag_query_func=None):
        """
        Args:
            ioap_send_func: IoAP 消息发送函数 async (msg: dict) -> dict
            dag_query_func: DAG 状态查询函数 async (dag_id: str) -> dict
        """
        self._ioap_send = ioap_send_func
        self._dag_query = dag_query_func
        # A2A Task ID → IoAP correlation_id 映射
        self._task_mapping: dict[str, str] = {}
        # IoAP correlation_id → A2A Task ID 反向映射
    ...
    async def send_task_as_ioap(self, task_params: TaskSendParams,
        ...
        实现双向映射：记录 A2A Task ID 和 IoAP correlation_id 的对应关系
        """
        ...
        for part in task_params.message.parts:
            if part.type == PartType.text and part.text:
                user_text += part.text
            elif part.type == PartType.data and part.data:
                structured_data.update(part.data)

        # 生成 correlation_id 并建立映射
        correlation_id = f"a2a-{task_params.id}"
        self._task_mapping[task_params.id] = correlation_id
        self._reverse_mapping[correlation_id] = task_params.id

        # 构造 IoAP 消息（包含完整的 A2A 上下文）
        ioap_msg = {
            ...
            "payload": {
                ...
                "source": "a2a",
                # A2A 接受的输出格式
                "accepted_output_modes": task_params.acceptedOutputModes,
            },
    ...
    async def sync_task_status(self, task_id: str, task_store) -> Task | None:
        """同步 A2A Task 状态与 IoAP DAG 状态

        查询对应的 DAG 状态，更新 A2A Task
        """
        if not self._dag_query:
            return None

        correlation_id = self._task_mapping.get(task_id)
        if not correlation_id:
            return None

        try:
            # 查询对应的 DAG 状态
            dag_status = await self._dag_query(correlation_id.replace("a2a-", ""))
            if not dag_status:
                return None

            # 映射 DAG 状态到 A2A Task 状态
            state_mapping = {
            ...
            a2a_state = state_mapping.get(dag_status.get("status"), TaskState.working)

            # 更新 Task 状态
            task = task_store.get_task(task_id)
            if task:
                task_store.update_task_status(task_id, a2a_state)

            return task

        except Exception as e:
            logger.warning("Failed to sync task status: %s", e)
            return None
