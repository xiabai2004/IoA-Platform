# IoA 分布式网络运维协同平台

> **Internet of Agents — 基于智能体互联网的跨域分布式任务协同调度系统**
>
> C4 网络技术挑战赛 B-EP1"智能体互联网创新攻关" | 2026

---

## 项目概述

本系统是一个**双层架构的智能体互联网（IoA）应用**：

- **下层（IoA中间件）**：自研通用多Agent通信与编排基础设施 — 语义消息路由总线、DAG任务调度引擎、Agent注册中心、闭环验证器、身份认证
- **上层（应用场景）**：分布式网络运维 — 5个跨域Agent（orchestrator、monitor-global、diagnoser、repairer、verifier）协同完成全网监控、故障诊断、自动修复、闭环验证

核心创新：不是做一个网络运维工具，而是**用网络运维场景证明IoA中间件的通用性和实用性**。系统从自然语言运维指令出发，通过语义路由自动匹配DAG模板，实现"意图理解→任务编排→协同执行→闭环验证"的全链路自动化。

---

## 系统特性

- **自然语言驱动**：用口语化指令触发复杂运维流程，无需API调用
- **语义消息路由**：Agent间通信基于能力匹配而非硬编码寻址，实现松耦合
- **DAG编排引擎**：Kahn拓扑排序 + 重试策略 + 审计日志，任务图自动解析执行
- **闭环验证**：三态判定（pass/retry/fail），实时指标兜底逻辑防止无限重试
- **多域协同**：华东/华北/华南/西南四域网络拓扑，故障注入→修复全链路
- **深色科技风GUI**：拓扑图实时状态、指标仪表盘、IoAP消息流、DAG可视化

---

## 技术栈

| 层次 | 技术 |
|------|------|
| 后端框架 | Python 3.12 + FastAPI |
| 中间件 | 自研IoA（注册中心、语义路由、DAG调度、闭环验证） |
| AI引擎 | LangGraph（Agent工作流）、DeepSeek API（LLM推理） |
| 数据存储 | SQLite（IoA元数据） |
| 网络模拟 | 自研多域网络仿真器（4域×12节点） |
| 进程管理 | systemd |
| 代理 | Nginx（SSL + WebSocket反代） |
| 前端 | 原生HTML5 + Chart.js + vis-network.js |
| 部署 | 阿里云ECS 2核2G + Ubuntu 22.04 |

---

## 架构总览

```
┌─────────────────────────────────────────────┐
│              GUI 控制台 (Material Symbols)    │
│   拓扑视图 │ 消息流 │ DAG可视化 │ 仪表盘     │
├─────────────────────────────────────────────┤
│               IoA 中间件层（自研）            │
│  ┌────────┐ ┌────────┐ ┌────────┐ ┌──────┐ │
│  │注册中心│ │语义路由│ │DAG调度│ │闭环验证│  │
│  └────────┘ └────────┘ └────────┘ └──────┘ │
├─────────────────────────────────────────────┤
│              Agent 运行时 (LangGraph)        │
│  orchestrator → monitor → diagnoser →       │
│  repairer → verifier                        │
├─────────────────────────────────────────────┤
│           集成 & 数据层                       │
│  网络模拟器 │ MCP服务器 │ 日志 │ SQLite      │
└─────────────────────────────────────────────┘
```

---

## 快速开始

### 环境要求

- Python 3.10+
- pip
- 系统内存 ≥ 1GB（模拟器 + 中间件约需 500MB）

### 安装

```bash
cd backend
pip install -r requirements.txt
```

### 启动（默认端口）

```bash
python run.py
```

服务启动后：
- 中间件 + GUI：http://127.0.0.1:8000
- 网络模拟器：http://127.0.0.1:8001
- GUI：http://127.0.0.1:8000/gui

### 一键体验

打开 GUI 后，点击 **▶ 一键演示** 按钮，系统自动执行：
1. 清除现有故障
2. 注入"华东地区链路拥塞"故障
3. 发送自然语言指令 → 触发 full_remediation DAG
4. 约6秒完成全链路闭环（监控→诊断→修复→验证）

---

## Agent 架构

| Agent | 职责 | 技术 |
|-------|------|------|
| **orchestrator** | 接收自然语言指令，意图识别→DAG模板匹配→任务分发 | LangGraph + 规则引擎 |
| **monitor-global** | 全网指标采集（四域），异常检测 | 模拟器API轮询 |
| **diagnoser** | 根因分析，多维指标交叉对比 | DeepSeek LLM |
| **repairer** | 自动修复执行，重试时自适应跳过已完成操作 | 状态机 |
| **verifier** | 闭环验证，三态判定 + 实时指标兜底 | 验证引擎 |

---

## DAG 工作流

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ monitor │ ──→ │diagnose │ ──→ │ repair  │ ──→ │ verify  │
└─────────┘     └─────────┘     └─────────┘     └─────────┘
   采集指标      根因分析       自动修复        闭环验证
   (5-10s)      (LLM推理)      (指令执行)      (三态判定)
```

full_remediation DAG 从注入故障到验证通过，端到端耗时约 **6秒**（链路拥塞场景）。

---

## 关键API

| 端点 | 方法 | 说明 |
|------|------|------|
| `/registry/agents` | GET | Agent列表（Bearer ioa2026demo） |
| `/dag` | POST | 创建DAG |
| `/dag/{dag_id}` | GET | 查询DAG状态 |
| `/messages` | POST | 发送IoAP消息（NL指令入口） |
| `/simulator/metrics` | GET | 全网实时指标 |
| `/simulator/fault/inject` | POST | 注入故障 |
| `/simulator/fault/clear_all` | GET | 清除所有故障 |

---

## 项目结构

```
IoA 分布式网络运维协同平台/
├── backend/
│   ├── ioa_middleware/        # IoA中间件核心
│   │   ├── main.py            # FastAPI 应用入口
│   │   ├── registry.py        # Agent注册中心
│   │   ├── router.py          # 语义消息路由总线
│   │   └── orchestrator/      # DAG调度引擎
│   │       ├── scheduler.py   # Kahn拓扑排序
│   │       └── templates.py   # DAG模板库
│   ├── agents/                # Agent实现
│   │   ├── __init__.py        # Agent工厂
│   │   ├── base_agent.py      # 基类
│   │   ├── orchestrator_agent.py
│   │   ├── monitor_agent.py
│   │   ├── diagnoser_agent.py
│   │   ├── repairer_agent.py
│   │   └── verifier_agent.py
│   ├── simulator/             # 网络模拟器
│   │   ├── api.py             # 模拟器API
│   │   └── network.py         # 拓扑与流量模型
│   ├── data/                  # SQLite数据库
│   └── run.py                 # 一键启动
├── gui/
│   └── index.html             # 控制台（Material Symbols图标）
├── docs/
│   ├── ARCHITECTURE.md        # 架构设计文档
│   ├── DEPLOYMENT.md          # 部署文档
│   └── DEMO_SCRIPT.md         # 演示视频脚本
├── requirements.txt
└── README.md
```

---

## 演示地址

**在线平台**：https://xiabai.site/gui

演示环境部署在阿里云ECS（2核2G，Ubuntu 22.04），5个Agent常驻运行。
认证令牌：`ioa2026demo`

---

## 团队

- 参赛赛项：B-EP1 智能体互联网创新攻关
- 选题方向：跨域分布式任务协同调度系统
- 技术关键词：IoA、语义路由、DAG编排、LangGraph、闭环验证